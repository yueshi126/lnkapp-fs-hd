...
import org.fbi.fshd.domain.cbs.T1000Request.CbsTia1000;
import org.fbi.fshd.domain.cbs.T1000Response.CbsToa1000;
import org.fbi.fshd.domain.cbs.T1000Response.CbsToa1000Item;
import org.fbi.fshd.domain.tps.T1000Request.TpsTia1000;
import org.fbi.fshd.domain.tps.T1000Response.TpsToa1000;
...
public class T1010Processor extends AbstractTxnProcessor {
    ...
    protected void doRequest(Stdp10ProcessorRequest request, Stdp10ProcessorResponse response) throws ProcessorException, IOException {
        ...
        //第三方通讯处理
        TpsTia1000 tpsTia = new TpsTia1000();
        TpsToa1000 tpsToa;
        ...
        try {
            ...
            tpsTia.setBillType("01");     //通知书类型
            tpsTia.setFisBatchSn("000001");   //批次号码信息
            tpsTia.setOutModeFlag("O"); //输出模式标识
            byte[] recvTpsBuf = processThirdPartyServer(marshalTpsRequestMsg(tpsTia), "1000");
    ...
    //解包生成CBS请求报文BEAN
    private CbsTia1000 unmarshalCbsRequestMsg(byte[] body) throws Exception {
        CbsTia1000 tia = new CbsTia1000();
        SeperatedTextDataFormat dataFormat = new SeperatedTextDataFormat(tia.getClass().getPackage().getName());
        tia = (CbsTia1000) dataFormat.fromMessage(new String(body, "GBK"), "CbsTia1000");
        return tia;
    }
    ...
    private String generateCbsRespMsgByLocalDbInfo(FsHdPaymentInfo paymentInfo, List<FsHdPaymentItem> paymentItems) {
        CbsToa1000 cbsToa = new CbsToa1000();
        FbiBeanUtils.copyProperties(paymentInfo, cbsToa);

        List<CbsToa1000Item> cbsToaItems = new ArrayList<>();
        for (FsHdPaymentItem paymentItem : paymentItems) {
            CbsToa1000Item cbsToaItem = new CbsToa1000Item();
    ...
    //组第三方服务器请求报文
    private byte[] marshalTpsRequestMsg(TpsTia1000 tpsTia) {
        Map<String, Object> modelObjectsMap = new HashMap<String, Object>();
    ...
    //解包生成第三方响应报文BEAN
    private TpsToa1000 unmarshalTpsResponseMsg(byte[] response) throws Exception {
        TpsToa1000 toa = new TpsToa1000();
        FixedLengthTextDataFormat dataFormat = new FixedLengthTextDataFormat(toa.getClass().getPackage().getName());
        toa = (TpsToa1000) dataFormat.fromMessage(response, "TpsToa1000");

        return toa;
    }

    //根据第三方服务器响应报文生成特色平台响应报文
    private String marshalCbsResponseMsg(TpsToa1000 tpsToa) {
        CbsToa1000 cbsToa = new CbsToa1000();
        FbiBeanUtils.copyProperties(tpsToa, cbsToa);
        List<CbsToa1000Item> cbsToa1000Items = new ArrayList<>();
        for (TpsToa1000Item tpstoaItem : tpsToa.getItems()) {
            CbsToa1000Item cbsToaItem = new CbsToa1000Item();
            FbiBeanUtils.copyProperties(tpstoaItem, cbsToaItem);
            cbsToa1000Items.add(cbsToaItem);
        }
        cbsToa.setItems(cbsToa1000Items);
    ...
    private void processTxn(TpsToa1000 tpsToa, Stdp10ProcessorRequest request) {
        ...
        try {
            ...
            int i = 0;
            for (TpsToa1000Item toaItem : tpsToa.getItems()) {
                i++;
